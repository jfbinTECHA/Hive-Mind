...
export class RelationshipSystem {
  ...
  private updateMetricsFromInteraction(interaction: InteractionEvent): void {
    ...
    if (type === 'emotional_sharing' || type === 'personal_story') {
      this.relationshipMetrics.intimacy = Math.min(
        1,
        this.relationshipMetrics.intimacy + emotionalDepth * 0.1
      );
    }
    ...
    if (type === 'shared_interest' || type === 'mutual_understanding') {
      this.relationshipMetrics.compatibility = Math.min(
        1,
        this.relationshipMetrics.compatibility + sharedInterests * 0.08
      );
    }

    // Update communication quality
    if (type === 'conversation' || type === 'deep_discussion') {
      this.relationshipMetrics.communication = Math.min(
        1,
        this.relationshipMetrics.communication + quality * 0.03
      );
    }

    // Update consistency
    this.relationshipMetrics.consistency = Math.min(
      1,
      this.relationshipMetrics.consistency + consistency * 0.02
    );
  }
  ...
  private applyTemporalDecay(): void {
    ...
    Object.keys(this.relationshipMetrics).forEach(key => {
      ...
      this.relationshipMetrics[metricKey] = Math.max(
        0.1,
        this.relationshipMetrics[metricKey] - decayRate
      );
    });
  }
  ...
  getCurrentRelationshipLevel(): RelationshipLevel {
    ...
    for (let i = this.relationshipLevels.length - 1; i >= 0; i--) {
      ...
      if (totalInteractions >= level.interactionThreshold && averageMetrics >= level.level * 0.15) {
        // 15% per level
        return level;
      }
    }
  ...
  getProgressToNextLevel(): {
    ...
    nextLevel?: RelationshipLevel;
    progress: number;
  } {
    const currentLevel = this.getCurrentRelationshipLevel();
    const nextLevelIndex = this.relationshipLevels.findIndex(
    ...
    if (nextLevel) {
      ...
      progress = Math.min(
        1,
        (currentInteractions - currentThreshold) / (nextThreshold - currentThreshold)
      );
    }
